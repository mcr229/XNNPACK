    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    const __m128 vksum0x0123 = _mm_load_ps((const float*) w);
    __m128i vinput_zero_point0_int = _mm_cvtsi32_si128(*((const int*) &quantization_params[0].zero_point));
    vinput_zero_point0_int = _mm_shuffle_epi32(vinput_zero_point0_int, _MM_SHUFFLE(0, 0, 0, 0));
    __m128 vinput_zero_point0 = _mm_cvtepi32_ps(vinput_zero_point0_int);

    __m128 vout0x0123 = _mm_mul_ps(vksum0x0123, vinput_zero_point0);
    ...
    __m128 one_sixteenth = _mm_load1_ps(&params->sse.one_sixteenth);
