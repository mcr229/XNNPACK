    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  do {
    ...
    const __m128i vinput_zero_point0_int = _mm_cvtsi32_si128(*((const int*) &quantization_params[0].zero_point));
    vinput_zero_point0_int = _mm_shuffle_epi32(vinput_zero_point0, _MM_SHUFFLE(0, 0, 0, 0));
    __m128 vinput_zero_point0 = _mm_cvtepi32_ps(vinput_zero_point0_int);
    __m128 vout0x0123 = _mm_mul_ps(vksum, vinput_zero_point0);
    w = (const int32_t*) w + 4;

    for (size_t nb=0; nb<n_blocks; ++nb){
      __m128i vacc0x0 = _mm_setzero_sil128();
      __m128i vacc0x1 = _mm_setzero_sil128();
      __m128i vacc0x2 = _mm_setzero_sil128();
      __m128i vacc0x3 = _mm_setzero_sil128();
      size_t k = bl;
    ...
    const __m128 voutput_max_less_zero_point = _mm_load_ps(params->sse.output_max_less_zero_point);
    vscaled0x0123 = _mm_min_ps(vscaled0x0123, voutput_max_less_zero_point);

    vacc0x0123 = _mm_cvtps_epi32(vscaled0x0123);

    const __m128i voutput_zero_point = _mm_load_si128((const __m128i*) params->sse.output_zero_point);
    __m128i vacc00x0123 = _mm_adds_epi16(_mm_packs_epi32(vacc0x0123, vacc0x0123), voutput_zero_point);

    const __m128i voutput_min = _mm_load_si128((const __m128i*) params->sse.output_min);
